...
def get_net_device_usage():
    ...
    try:
        ...
        truncate_usage = "DELETE FROM cradlepoint.net_device_usage WHERE DATE_TRUNC('day',created_at) >= current_date - interval '1 day'"
        ...
        # Get last successful net_device_usage sync
        pg_date = "select max(next_run) as next_run from cradlepoint.run_history where entity = 'net_device_usage_samples'"
        cursor.execute(pg_date)
        max_date = cursor.fetchone()[0]
        USAGE_START_DATE = max_date.strftime("%Y-%m-%d")
        ...
        for net_devices in chunker(sorted(net_device_ids), API_OBJ_REQUEST_SIZE):
            ...
            url += '&created_at__gt={}'.format(USAGE_START_DATE)
                ...
                while url:
                    ...
                    if (len(resp['data']) > 0):
                        for usage in resp['data']:
                            ...
                            insert = 'INSERT INTO cradlepoint.net_device_usage (%s) VALUES %s'
                            cursor.execute(insert, (AsIs(','.join(columns)), tuple(values)))

        # Insert usage_samples timestamp into run_history
        ...
        cursor.execute(insert, ('net_device_usage_samples',ulast_run, unext_run))
