...
:email
  echo Send-MailMessage -From "!pbi_email!" -To "!error_email!" -Subject "QQube-PostgreSQL Sync Failure" -Body "This is an automated alert. The script which automatically syncs QQube to PostgreSQL failed to execute. See the log file for more details." -SmtpServer "smtp-mail.outlook.com" -Port 587 -UseSsl -Credential ^(New-Object PSCredential^("!pbi_email!", ^(ConvertTo-SecureString "!pbi_password!" -AsPlainText -Force^)^)^)>"%script%"
